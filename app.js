...
const MODULOS = [moduloDashboard, moduloHSK, moduloFB, moduloNautico, moduloServicos, moduloAlojamento, moduloEscalas, moduloParametros, moduloOrcamento, moduloAtualizacoes];
...
function montarNavegacao(conta) {
  ...
  MODULOS.forEach(m => {
    nav.append(el("button", {
      class: "tab", "data-id": m.id,
      onclick: () => abrir(m.id)
    }, icone(m.id), el("span", {}, m.nome)));
  });
  // separador de Zonas (transversal)
  nav.append(el("button", {
    class: "tab", "data-id": "zonas",
    onclick: () => abrirZonas()
  }, icone("zonas"), el("span", {}, "Zonas")));
