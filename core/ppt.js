...
// mês de maior necessidade (pico) do plano para um grupo de departamentos
function mesPicoPlano(D, depsNorm) {
  if (!D.plano) return null;
  let idx = D.mesIdx, melhor = -1;
  MESES.forEach((_, i) => { const h = planoHC(D, depsNorm, i); if (h > melhor) { melhor = h; idx = i; } });
  return { idx, hc: Math.round(melhor) };
}

// nomes de mês completos (para casar com a revisão F&B)
const NOMES_MES = ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"];

// ---- plano "com revisão": para o F&B usa a revisão (revisao_fb.json) quando
// existe; caso contrário (e p/ outros departamentos) usa a Lista Orçamento.
// A revisão tem duas versões: original e revisto. O plano principal = revisto. -
function revVer(D, ver) { return D.revFB && D.revFB.versoes && D.revFB.versoes[ver]; }
function temPlanoMod(D, C) { return !!((C.revisao && D.revFB) || D.temPlano); }
function planoHCmod(D, C, mesIdx) {
  const v = C.revisao && revVer(D, "revisto");
  if (v) return Math.round(Number(v.totalMes[NOMES_MES[mesIdx]]) || 0);
  return D.temPlano ? Math.round(planoHC(D, C.deps, mesIdx)) : null;
}
function planoHCorig(D, C, mesIdx) {
  const v = C.revisao && revVer(D, "original");
  if (v) return Math.round(Number(v.totalMes[NOMES_MES[mesIdx]]) || 0);
  return null;
}
function planoFuncMod(D, C, mesIdx, ver) {
  const v = C.revisao && revVer(D, ver || "revisto");
  if (v) return { ...(v.porFuncao[NOMES_MES[mesIdx]] || {}) };
  return D.temPlano ? planoPorFuncao(D, C.deps, mesIdx) : {};
}
function picoMod(D, C) {
  const v = C.revisao && revVer(D, "revisto");
  if (v) {
    let idx = D.mesIdx, melhor = -1;
    NOMES_MES.forEach((nm, i) => { const h = Number(v.totalMes[nm]) || 0; if (h > melhor) { melhor = h; idx = i; } });
    return { idx, hc: Math.round(melhor) };
  }
  return D.temPlano ? mesPicoPlano(D, C.deps) : null;
}
function serieMod(D, C, ver) {
  const v = C.revisao && revVer(D, ver || "revisto");
  if (v) return NOMES_MES.map(nm => Math.round(Number(v.totalMes[nm]) || 0));
  return MESES.map((_, i) => Math.round(planoHC(D, C.deps, i)));
}
// slide/painel de sazonalidade: HC do plano por mês, com atual e pico destacados
function painelSazonalidade(F, pptx, s, D, serie, picoIdx, efetivo, yTop, serieOrig) {
  const y0 = yTop || 2.05;
  const maxV = Math.max(1, ...serie, ...(serieOrig || []));
  const x0 = F.MARGEM, larg = F.LARG, baseY = y0 + 3.6, altMax = 3.2;
  const passo = larg / 12, bw = passo * 0.62;
  serie.forEach((v, i) => {
    const h = Math.max(0.03, v / maxV * altMax);
    const x = x0 + i * passo + (passo - bw) / 2;
    const atual = i === D.mesIdx, ehPico = picoIdx != null && i === picoIdx;
    const cor = atual ? "11857A" : ehPico ? "A6824A" : "DDE3DC";
    s.addShape(pptx.ShapeType.roundRect, { x, y: baseY - h, w: bw, h, fill: { color: cor }, line: { type: "none" }, rectRadius: 0.02 });
    // contorno da versão original (referência), quando fornecida
    if (serieOrig && serieOrig[i] != null) {
      const ho = Math.max(0.03, serieOrig[i] / maxV * altMax);
      s.addShape(pptx.ShapeType.rect, { x, y: baseY - ho, w: bw, h: ho, fill: { type: "none" }, line: { color: "5E6B67", width: 0.75, dashType: "dash" } });
    }
    s.addText("" + v, { x: x - 0.1, y: baseY - h - 0.3, w: bw + 0.2, h: 0.28, fontSize: 10, bold: atual || ehPico, color: atual ? "11857A" : ehPico ? "84693B" : "5E6B67", align: "center" });
    s.addText(MESES[i], { x: x - 0.1, y: baseY + 0.06, w: bw + 0.2, h: 0.26, fontSize: 10, bold: atual || ehPico, color: atual || ehPico ? "19211F" : "92A09A", align: "center" });
  });
  // linha do efetivo atual
  if (efetivo != null && efetivo <= maxV) {
    const yL = baseY - efetivo / maxV * altMax;
    s.addShape(pptx.ShapeType.line, { x: x0, y: yL, w: larg, h: 0, line: { color: "B34B4B", width: 1, dashType: "dash" } });
    s.addText("efetivo atual: " + efetivo, { x: x0, y: yL - 0.28, w: 3, h: 0.26, fontSize: 10, italic: true, color: "B34B4B" });
  }
  // legenda
  s.addText([
    { text: "■ ", options: { color: "11857A" } }, { text: "mês atual   ", options: { color: "5E6B67" } },
    { text: "■ ", options: { color: "A6824A" } }, { text: "pico (maior necessidade)", options: { color: "5E6B67" } }
  ], { x: x0, y: baseY + 0.4, w: larg, h: 0.3, fontSize: 10 });
}
...
async function recolher() {
  ...
  let presencas = [], camas = [], budget = null, revFB = null;
  ...
  try { revFB = await (await fetch("revisao_fb.json?" + Date.now())).json(); } catch {}
  ...
  return { mesIso, mesIdx: HOJE().getMonth(), presencas, camas, assid, aloj, custo, temBudget: !!budget, plano, planoCol, temPlano: !!plano, revFB };
}
...
async function gerarFB(pptx, D) {
  await deckDepartamento(pptx, D, {
    ...
    modulo: "F&B", deps: DEPS_FB, revisao: true,
    nota: "Composição da equipa, plano (revisão de headcount) vs efetivo, pipeline e carências por função."
  });
}
...
async function deckDepartamento(pptx, D, C) {
  ...
  // plano vs real (revisto = principal; original = referência quando há revisão)
  const planoTot = temPlanoMod(D, C) ? planoHCmod(D, C, mesIdx) : null;
  const planoOrig = C.revisao ? planoHCorig(D, C, mesIdx) : null;
  const pico = temPlanoMod(D, C) ? picoMod(D, C) : null;
  const planoPico = pico ? pico.hc : null;
  const planoPicoOrig = C.revisao && pico ? planoHCorig(D, C, pico.idx) : null;
  const vagasPico = planoPico != null ? Math.max(0, planoPico - ps.length) : null;
  ...
  let n = 0; const TOT = 10;

  // 1) CAPA
  let s = pptx.addSlide(); n++;
  F.capa(s, C.eyebrow, C.titulo, C.nota, [
    { v: planoTot != null ? planoTot : "—", l: "Plano (" + MESES[mesIdx] + ")" },
    { v: ps.length, l: "Pessoas" },
    { v: planoPico != null ? planoPico : "—", l: "Pico (" + (pico ? MESES[pico.idx] : "—") + ")" },
    { v: vagas != null ? vagas : "—", l: "Vagas" },
  ]);

  // 2) VISÃO GERAL — 6 KPIs (mês atual + pico)
  s = pptx.addSlide(); n++; s.background = { color: PAPER };
  ...
  const nomePico = pico ? MESES[pico.idx] : "—";
  F.kpi(s, F.MARGEM, 1.85, 3.85, planoTot != null ? planoTot : "—", "Plano (" + MESES[mesIdx] + ")", planoOrig != null ? ("revisto · original " + planoOrig) : (vagas != null ? (vagas + " vagas em aberto") : "sem orçamento"), GOLDD);
  ...
  F.kpi(s, F.MARGEM, 3.55, 3.85, planoPico != null ? planoPico : "—", "Pico — " + nomePico, planoPicoOrig != null ? ("revisto · original " + planoPicoOrig) : (vagasPico != null ? (vagasPico + " vagas no pico") : "maior necessidade"), GOLDD);
  F.kpi(s, F.MARGEM + 4.1, 3.55, 3.85, aChegar, "A chegar", "próximos a iniciar");
  F.kpi(s, F.MARGEM + 8.2, 3.55, 4.03, assidPct, "Assiduidade", a.P + " presenças no mês", TEAL2);
  s.addText("Plano = orçamento congelado. Mês atual (" + MESES[mesIdx] + ") e mês de maior necessidade (" + nomePico + "). Vagas = plano − pessoas atuais.", { x: F.MARGEM, y: 5.5, w: F.LARG, h: 0.4, fontSize: 12, italic: true, color: MUT });
  F.rodape(s, n, TOT);

  // 3) SAZONALIDADE — necessidade ao longo do ano
  s = pptx.addSlide(); n++; s.background = { color: PAPER };
  F.cabSeccao(s, "Sazonalidade", "Necessidade ao longo do ano", "Headcount do plano por mês — atual vs pico");
  if (temPlanoMod(D, C)) painelSazonalidade(F, pptx, s, D, serieMod(D, C), pico ? pico.idx : null, ps.length, 2.05, C.revisao ? serieMod(D, C, "original") : null);
  else s.addText("Sem Lista Orçamento — não é possível mostrar a curva anual.", { x: F.MARGEM, y: 2.3, w: F.LARG, h: 0.6, fontSize: 15, color: MUT });
  F.rodape(s, n, TOT);
  ...
  // 7) PLANO VS EFETIVO POR FUNÇÃO (mês atual + pico)
  s = pptx.addSlide(); n++; s.background = { color: PAPER };
  const subT = C.revisao ? "Necessidade original vs revista e efetivo, por função — pico de " + nomePico
    : "Plano de " + MESES[mesIdx] + " e do pico (" + nomePico + "), efetivo atual e vagas no pico";
  F.cabSeccao(s, "Plano vs efetivo", "Cobertura por função", subT);
  const realF = funcoesDe(ps);
  const planoF = temPlanoMod(D, C) ? planoFuncMod(D, C, mesIdx) : {};
  const planoFPico = temPlanoMod(D, C) && pico ? planoFuncMod(D, C, pico.idx) : {};
  const planoFPicoOrig = C.revisao && pico ? planoFuncMod(D, C, pico.idx, "original") : {};
  const chaves = new Set([...realF.map(x => normTxt(x[0])), ...Object.keys(planoF).map(normTxt), ...Object.keys(planoFPico).map(normTxt), ...Object.keys(planoFPicoOrig).map(normTxt)]);
  ...
  const picoMap = {}; Object.entries(planoFPico).forEach(([nm, q]) => picoMap[normTxt(nm)] = { nome: nm, q });
  const origMap = {}; Object.entries(planoFPicoOrig).forEach(([nm, q]) => origMap[normTxt(nm)] = { nome: nm, q });
  const linhasTab = [];
  for (const k of chaves) {
    const nome = realMap[k]?.nome || picoMap[k]?.nome || planoMap[k]?.nome || origMap[k]?.nome || k;
    const pl = Math.round(planoMap[k]?.q || 0), plPico = Math.round(picoMap[k]?.q || 0), plOrig = Math.round(origMap[k]?.q || 0), re = realMap[k]?.q || 0;
    linhasTab.push({ nome, pl, plPico, plOrig, re, vaga: Math.max(0, pl - re), vagaPico: Math.max(0, plPico - re) });
  }
  linhasTab.sort((x, y) => y.vagaPico - x.vagaPico || y.plPico - x.plPico);
  let rows;
  if (C.revisao) {
    rows = [F.cab(["Função", "Orig.", "Revisto", "Efetivo", "Vagas"])];
    linhasTab.slice(0, 11).forEach(r => rows.push([r.nome, "" + r.plOrig, "" + r.plPico, "" + r.re,
      { text: r.vagaPico ? "" + r.vagaPico : "—", options: { color: r.vagaPico ? DANGER : MUT, bold: r.vagaPico > 0 } }]));
  } else {
    rows = [F.cab(["Função", "Plano " + MESES[mesIdx], "Plano " + nomePico, "Efetivo", "Vagas " + nomePico])];
    linhasTab.slice(0, 11).forEach(r => rows.push([r.nome, "" + r.pl, "" + r.plPico, "" + r.re,
      { text: r.vagaPico ? "" + r.vagaPico : "—", options: { color: r.vagaPico ? DANGER : MUT, bold: r.vagaPico > 0 } }]));
  }
  F.tabela(s, rows, { x: F.MARGEM, w: F.LARG, y: 1.95, rowH: 0.42, colW: [5.4, 1.85, 1.85, 1.55, 1.58], align: "left" });
  F.rodape(s, n, TOT);

  // 8) FUNÇÕES COM MAIOR CARÊNCIA — no pico (mês mais exigente)
  s = pptx.addSlide(); n++; s.background = { color: PAPER };
  F.cabSeccao(s, "Carências", "Funções com maior carência", "Vagas por função no pico de " + nomePico + " (plano − efetivo)");
  const gaps = linhasTab.filter(r => r.vagaPico > 0).slice(0, 9).map(r => [r.nome, r.vagaPico]);
  if (gaps.length) {
    F.barras(s, F.MARGEM, 2.0, 7.6, 4.5, "Vagas por função (" + nomePico + ")", gaps.slice().reverse(), DANGER);
    const top = gaps[0];
    const gapAtual = linhasTab.reduce((sm, r) => sm + r.vaga, 0);
    F.nota(s, 8.55, 2.0, F.W - F.MARGEM - 8.55, 2.1, "Maior carência", top[0] + " precisa de mais " + top[1] + " pessoa(s) no pico de " + nomePico + ".");
    const totalGap = gaps.reduce((s2, g) => s2 + g[1], 0);
    F.nota(s, 8.55, 4.35, F.W - F.MARGEM - 8.55, 2.15, "Atual vs pico", "Hoje (" + MESES[mesIdx] + ") faltam " + gapAtual + " pessoas; no pico de " + nomePico + " serão " + totalGap + ". Recrutar e escalar a tempo do pico.");
  } else {
    s.addText(D.temPlano ? "Sem carências face ao plano." : "Sem Lista Orçamento — não é possível calcular carências.", { x: F.MARGEM, y: 2.3, w: F.LARG, h: 0.6, fontSize: 15, color: MUT });
  }
  F.rodape(s, n, TOT);
  ...
  const resumo = [
    { v: emEscala, l: "Em escala", s: "a operar" + (planoTot != null ? " · plano " + MESES[mesIdx] + " " + planoTot : "") },
    { v: vagas != null ? vagas : "—", l: "Vagas hoje", s: vagas ? "recrutamento em curso" : "plano coberto" },
    { v: vagasPico != null ? vagasPico : "—", l: "Vagas no pico (" + nomePico + ")", s: "preparar a tempo do pico" },
  ];
...
async function gerarHSK(pptx, D) {
  ...
  const pico = D.temPlano ? mesPicoPlano(D, DEPS_HSK) : null;
  const planoPico = pico ? pico.hc : null;
  const nomePico = pico ? MESES[pico.idx] : "—";
  const vagas = planoTot != null ? Math.max(0, planoTot - ps.length) : null;
  const vagasPico = planoPico != null ? Math.max(0, planoPico - ps.length) : null;
  ...
  F.capa(s, "Verdelago · Housekeeping", "Análise de Gestão da Equipa", "Composição, estrutura de vínculos e dependência de fornecedores", [
    ...
    { v: planoPico != null ? planoPico : "—", l: "Pico (" + nomePico + ")" },
    { v: vagasPico != null ? vagasPico : "—", l: "Vagas no pico" },
  ]);
  ...
  const leituras = [
    ...
    { v: vagasPico != null ? vagasPico : "—", l: "Pico — " + nomePico, s: planoPico != null ? ("Plano sobe a " + planoPico + " no pico de " + nomePico + (vagasPico ? " — faltam " + vagasPico + " face ao efetivo atual." : " — coberto.")) : "Sem Lista Orçamento." },
  ];
  ...
  // 6) SAZONALIDADE — necessidade ao longo do ano
  s = pptx.addSlide(); n++; s.background = { color: PAPER };
  F.cabSeccao(s, "Sazonalidade", "Necessidade ao longo do ano", "Headcount do plano por mês — atual (" + MESES[mesIdx] + ") vs pico (" + nomePico + ")");
  if (D.temPlano) painelSazonalidade(F, pptx, s, D, MESES.map((_, i) => Math.round(planoHC(D, DEPS_HSK, i))), pico ? pico.idx : null, ps.length, 2.05);
  else s.addText("Sem Lista Orçamento — não é possível mostrar a curva anual.", { x: F.MARGEM, y: 2.3, w: F.LARG, h: 0.6, fontSize: 15, color: MUT });
  F.rodape(s, n, TOT);

  // 7) ESTRUTURA OPERACIONAL — 2 cartões (interna / externa) com funções
  s = pptx.addSlide(); n++; s.background = { color: PAPER };
...
// nome interno do campo de data de entrada. Varre TODAS as pessoas (o Graph
// não devolve campos vazios, por isso a 1ª pessoa pode não ter a coluna) e
// aceita variações (DataEntrada, Data Admissão, Data Início…), excluindo saída.
let _campoEnt;
function campoDataEntrada() {
  if (_campoEnt !== undefined) return _campoEnt;
  const ent = /entrad|admiss|inici|chegad|in[ií]cio/i, sai = /said|sa[ií]da|fim|termo/i;
  for (const p of dados.pessoas) {
    const k = Object.keys(p).find(k => /data|date|dt/i.test(k) && ent.test(k) && !sai.test(k));
    if (k) return (_campoEnt = k);
  }
  for (const p of dados.pessoas) {
    const k = Object.keys(p).find(k => ent.test(k) && !sai.test(k));
    if (k) return (_campoEnt = k);
  }
  return (_campoEnt = null);
}
