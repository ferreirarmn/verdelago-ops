...
async function recolher() {
  ...
  return { mesIso, presencas, camas, assid, aloj, custo, temBudget: !!budget };
}
...
async function gerarFB(pptx, D) {
  const F = fabrica(pptx);
  const ps = pessoasDoModulo("F&B").filter(ativa);
  const a = D.assid["F&B"], c = D.custo["F&B"];
  ...
  let s = pptx.addSlide(); n++;
  F.capa(s, "Verdelago · F&B", "Food & Beverage", "Composição da equipa, vínculos e assiduidade", [
    { v: ps.length, l: "Colaboradores" },
    { v: assidPct, l: "Assiduidade" },
    { v: D.aloj["F&B"], l: "Alojados" },
    { v: porEmp.length, l: "Empresas" },
  ]);

  // composição: KPIs + donuts
  s = pptx.addSlide(); n++; s.background = { color: PAPER };
  F.cabSeccao(s, "Composição da equipa", "Quem somos no F&B", "Distribuição por empresa e por vínculo contratual");
  const tt = ps.filter(p => classeVinculo(p.Vinculo) === "TT").length;
  F.filaKpis(s, 1.85, [
    { v: ps.length, l: "Colaboradores", s: "ativos no F&B" },
    { v: pct(tt, ps.length) + "%", l: "Trabalho temporário", s: tt + " via agência" },
    { v: porEmp.length, l: "Empresas", s: "fornecedores distintos" },
    { v: assidPct, l: "Assiduidade", s: a.P + " presenças no mês", cor: TEAL2 },
  ]);
  F.donut(s, F.MARGEM, 3.7, 5.9, 3.0, "Por empresa / agência", porEmp, ps.length);
  F.donut(s, F.MARGEM + 6.3, 3.7, 5.9, 3.0, "Por vínculo", porVinc, ps.length);
  F.rodape(s, n, TOT);

  // distribuição por função (barras) + nota
  s = pptx.addSlide(); n++; s.background = { color: PAPER };
  F.cabSeccao(s, "Distribuição por função", "Onde está concentrada a equipa", "Número de colaboradores por função operacional");
  const porFunc = funcoesDe(ps).slice(0, 9);
  F.barras(s, F.MARGEM, 1.95, 7.6, 4.7, "Colaboradores por função", porFunc.reverse(), TEAL2);
  const top = funcoesDe(ps)[0];
  F.nota(s, 8.55, 1.95, F.W - F.MARGEM - 8.55, 2.2, "Função dominante",
    top ? (top[0] + " concentra " + top[1] + " dos " + ps.length + " colaboradores (" + pct(top[1], ps.length) + "%) — o núcleo da operação de sala e cozinha.") : "Sem dados de função.");
  F.nota(s, 8.55, 4.35, F.W - F.MARGEM - 8.55, 2.3, "Leitura",
    "A equipa reparte-se por " + porFunc.length + " funções. Acompanhar o equilíbrio entre níveis (1ª/2ª) é o que mais influencia a qualidade de serviço e o custo.");
  F.rodape(s, n, TOT);

  // tabela operacional por empresa
  s = pptx.addSlide(); n++; s.background = { color: PAPER };
  F.cabSeccao(s, "Estrutura operacional", "Equipa por empresa e vínculo", "Detalhe da composição atual do F&B");
  const linhas = [F.cab(["Empresa / agência", "Pessoas", "Quadro", "TT", "Outros"])];
  const grupos = {};
  ps.forEach(p => { const e = (p.EmpresaAgencia || "—").trim() || "—"; const v = classeVinculo(p.Vinculo); (grupos[e] ||= { tot: 0, q: 0, tt: 0, o: 0 }); grupos[e].tot++; if (v === "Quadro") grupos[e].q++; else if (v === "TT") grupos[e].tt++; else grupos[e].o++; });
  Object.entries(grupos).sort((x, y) => y[1].tot - x[1].tot).forEach(([e, g]) => linhas.push([e, "" + g.tot, "" + g.q, "" + g.tt, "" + g.o]));
  F.tabela(s, linhas, { x: F.MARGEM, w: F.LARG, y: 1.95, rowH: 0.44, colW: [5.1, 1.8, 1.7, 1.6, 2.0], align: "left" });
  F.rodape(s, n, TOT);
}
...
async function gerarHSK(pptx, D) {
  ...
  const a = D.assid["HSK"], c = D.custo["HSK"];
  const assidPct = (a.P + a.F) ? pct(a.P, a.P + a.F) + "%" : "—";
  const porEmp = porChave(ps, "EmpresaAgencia").slice(0, 6);
  const porVinc = porChave(ps.map(p => ({ Vinculo: classeVinculo(p.Vinculo) })), "Vinculo");
  const tt = ps.filter(p => classeVinculo(p.Vinculo) === "TT").length;
  ...
  let s = pptx.addSlide(); n++;
  F.capa(s, "Verdelago · Housekeeping", "Housekeeping", "Composição, estrutura de vínculos e dependência de fornecedores", [
    { v: ps.length, l: "Colaboradores" },
  ...
  // sumário executivo (4 leituras) — cartões
  s = pptx.addSlide(); n++; s.background = { color: PAPER };
  F.cabSeccao(s, "Sumário executivo", "Quatro leituras essenciais", "");
  const topExt = externos[0];
  const leituras = [
    { v: ps.length, l: "Dimensão da equipa", s: ps.length + " colaboradores afetos ao Housekeeping." },
    { v: pct(tt, ps.length) + "%", l: "Externalização", s: tt + " dos " + ps.length + " são trabalho temporário." },
    { v: topExt ? pct(topExt[1], tt) + "%" : "—", l: "Concentração num fornecedor", s: topExt ? (topExt[0] + " concentra " + topExt[1] + " dos " + tt + " externos.") : "Sem TT." },
    { v: assidPct, l: "Assiduidade no mês", s: a.P + " presenças · " + a.F + " faltas." },
  ];
  const gap = 0.3, w = (F.LARG - gap) / 2, h = 1.85;
  leituras.forEach((r, i) => {
    const x = F.MARGEM + (i % 2) * (w + gap), y = 2.05 + Math.floor(i / 2) * (h + 0.3);
    F.bloco(s, x, y, w, h, CARD);
    s.addText("" + r.v, { x: x + 0.25, y: y + 0.22, w: w - 0.5, h: 0.8, fontSize: 36, bold: true, color: i === 1 ? GOLDD : TEAL, fontFace: "Cambria" });
    s.addText(r.l, { x: x + 0.25, y: y + 0.95, w: w - 0.5, h: 0.35, fontSize: 14, bold: true, color: INK, fontFace: "Cambria" });
    s.addText(r.s, { x: x + 0.25, y: y + 1.3, w: w - 0.5, h: 0.5, fontSize: 12, color: MUT, valign: "top" });
  });
  F.rodape(s, n, TOT);

  // distribuição por função (barras)
  s = pptx.addSlide(); n++; s.background = { color: PAPER };
  F.cabSeccao(s, "Distribuição por função", "Onde está concentrada a equipa", "Número de colaboradores por função");
  const porFunc = funcoesDe(ps).slice(0, 9);
  F.barras(s, F.MARGEM, 1.95, 7.6, 4.7, "Colaboradores por função", porFunc.slice().reverse(), TEAL2);
  const top = porFunc[0];
  F.nota(s, 8.55, 1.95, F.W - F.MARGEM - 8.55, 2.2, "Limpeza de andares domina",
    top ? (top[0] + " representa " + top[1] + " dos " + ps.length + " (" + pct(top[1], ps.length) + "%) — o coração da operação.") : "Sem dados.");
  F.nota(s, 8.55, 4.35, F.W - F.MARGEM - 8.55, 2.3, "Coordenação",
    "As funções de chefia coordenam a operação diária. Amplitude de controlo elevada pede supervisão atenta nos picos.");
  F.rodape(s, n, TOT);

  // dependência de fornecedores — donut + nota de risco
  s = pptx.addSlide(); n++; s.background = { color: PAPER };
...
function funcoesDe(pessoas) {
  ...
  pessoas.forEach(p => {
    let nome = "—";
    const v = p.FuncaoID;
    if (typeof v === "string" && v) { const f = dados.funcoes.find(f => f.Title === v); nome = f?.Nome || v; }
    else if (v && typeof v === "object" && v.LookupValue) nome = v.LookupValue;
    else if (p.FuncaoIDLookupId != null) { const f = dados.funcoes.find(f => String(f._id) === String(p.FuncaoIDLookupId)); nome = f?.Nome || "—"; }
    m[nome] = (m[nome] || 0) + 1;
  });
  return Object.entries(m).sort((a, b) => b[1] - a[1]);
}
