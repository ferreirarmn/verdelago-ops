...
function painelSazonalidade(F, pptx, s, D, depsNorm, efetivo, yTop) {
  const y0 = yTop || 2.05;
  const serie = MESES.map((_, i) => Math.round(planoHC(D, depsNorm, i)));
  const maxV = Math.max(1, ...serie);
  const x0 = F.MARGEM, larg = F.LARG, baseY = y0 + 3.6, altMax = 3.2;
  const passo = larg / 12, bw = passo * 0.62;
  const pico = mesPicoPlano(D, depsNorm);
  serie.forEach((v, i) => {
    const h = Math.max(0.03, v / maxV * altMax);
    const x = x0 + i * passo + (passo - bw) / 2;
    const atual = i === D.mesIdx, ehPico = pico && i === pico.idx;
...
async function deckDepartamento(pptx, D, C) {
  ...
  const pico = D.temPlano ? mesPicoPlano(D, C.deps) : null;
  ...
  F.kpi(s, F.MARGEM, 1.85, 3.85, planoTot != null ? planoTot : "—", "Plano (" + MESES[mesIdx] + ")", vagas != null ? (vagas + " vagas em aberto") : "sem orçamento", GOLDD);
  ...
  F.kpi(s, F.MARGEM, 3.55, 3.85, planoPico != null ? planoPico : "—", "Pico — " + nomePico, vagasPico != null ? (vagasPico + " vagas no pico") : "maior necessidade", GOLDD);
  ...
  if (D.temPlano) painelSazonalidade(F, pptx, s, D, C.deps, ps.length, 2.05);
  ...
  s = pptx.addSlide(); n++; s.background = { color: PAPER };
  F.cabSeccao(s, "Plano vs efetivo", "Cobertura por função", "Plano de " + MESES[mesIdx] + " e do pico (" + nomePico + "), efetivo atual e vagas no pico");
  const realF = funcoesDe(ps);
  const planoF = D.temPlano ? planoPorFuncao(D, C.deps, mesIdx) : {};
  const planoFPico = D.temPlano && pico ? planoPorFuncao(D, C.deps, pico.idx) : {};
  const chaves = new Set([...realF.map(x => normTxt(x[0])), ...Object.keys(planoF).map(normTxt), ...Object.keys(planoFPico).map(normTxt)]);
  ...
  for (const k of chaves) {
    const nome = realMap[k]?.nome || planoMap[k]?.nome || picoMap[k]?.nome || k;
    const pl = Math.round(planoMap[k]?.q || 0), plPico = Math.round(picoMap[k]?.q || 0), re = realMap[k]?.q || 0;
    linhasTab.push({ nome, pl, plPico, re, vaga: Math.max(0, pl - re), vagaPico: Math.max(0, plPico - re) });
  }
  linhasTab.sort((x, y) => y.vagaPico - x.vagaPico || y.plPico - x.plPico);
  const rows = [F.cab(["Função", "Plano " + MESES[mesIdx], "Plano " + nomePico, "Efetivo", "Vagas " + nomePico])];
...
async function gerarHSK(pptx, D) {
  ...
  if (D.temPlano) painelSazonalidade(F, pptx, s, D, DEPS_HSK, ps.length, 2.05);
  else s.addText("Sem Lista Orçamento — não é possível mostrar a curva anual.", { x: F.MARGEM, y: 2.3, w: F.LARG, h: 0.6, fontSize: 15, color: MUT });
  F.rodape(s, n, TOT);
