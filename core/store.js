...
/** Módulos operacionais (os restantes caem em "Serviços Centrais"). */
export const MODULOS_OPERACIONAIS = ["HSK", "F&B", "Náutico"];

/** Turno (objeto) pelo TurnoID. */
export const turnoPorId = id => dados.turnos.find(t => t.Title === id) || null;

/** Pessoas que não pertencem a nenhum módulo operacional (Serviços Centrais). */
export function pessoasGeral(incluirInativas = false) {
  return dados.pessoas.filter(p => {
    if (!incluirInativas && (p.Estado || "").toLowerCase().includes("inativ")) return false;
    return !MODULOS_OPERACIONAIS.includes(moduloDaPessoa(p));
  });
}
