...
function exigirLista(nome) {
  let id = idDaLista[nome];
  if (!id) {
    // tolerante a acentos/maiúsculas/espaços (ex.: "Correspondencias" vs "Correspondências")
    const alvo = String(nome || "").toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g, "").replace(/\s+/g, " ").trim();
    for (const dn in idDaLista) {
      if (dn.toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g, "").replace(/\s+/g, " ").trim() === alvo) { id = idDaLista[dn]; break; }
    }
  }
  if (!id) throw new Error('Lista "' + nome + '" não encontrada no site. Listas existentes: ' + Object.keys(idDaLista).join(", "));
  return id;
}
