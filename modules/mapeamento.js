...
// nomes distintos no REAL: departamentos (das pessoas) e funções (Lista Funções)
function nomesReal(tipo) {
  if (tipo === "Departamento") {
    return [...new Set(dados.pessoas.map(p => moduloDaPessoa(p)).filter(Boolean))].sort();
  }
...
function vista(corpo, self, core, alvo) {
  ...
  const itens = itensCorrespondencia().filter(it => (it.Tipo || "Função") === tipo);
  const mapPlano = {}; itens.forEach(it => mapPlano[normCorr(it.Title)] = it);

  // nomes do plano ainda sem correspondência
  const porMapear = plano.filter(nm => !mapPlano[normCorr(nm)]);

  const aviso = el("div", { class: "mod-nota", style: porMapear.length ? "border-left:3px solid var(--gold)" : "" },
    porMapear.length
      ? (porMapear.length + " nome(s) do orçamento ainda sem correspondência ao real. Resolve-os para os números baterem certo.")
      : "Todos os nomes do orçamento têm correspondência ao real. ✓");
  ...
  const sugestoes = porMapear.map(nm => {
    ...
    real.forEach(r => { const s = semelhanca(nm, r); if (s > sc) { sc = s; melhor = r; } });
    return el("tr", {},
      el("td", {}, nm),
      el("td", {}, melhor && sc >= 0.5 ? el("span", {}, "sugestão: ", el("b", {}, melhor)) : el("span", { class: "mut" }, "sem sugestão")),
      el("td", {}, el("button", { class: "btn-sec", onclick: () => editar({ Title: nm, Tipo: tipo, NomeReal: (sc >= 0.5 ? melhor : "") }, self, core, alvo) }, "Mapear")));
  });
...
function editar(item, self, core, alvo) {
  ...
  const guardar = async () => {
    ...
    try {
      badge("syncing");
      const campos = { Title: plano, Tipo: tipo, NomeReal: realNome };
