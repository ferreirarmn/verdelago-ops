...
import { pessoasDoModulo, funcaoDaPessoa } from "../core/store.js";
...
function prepararPessoas() {
  ...
  return pessoasDoModulo("HSK").map(p => {
    ...
    return { nome: p.Nome, empresa: p.EmpresaAgencia || "", funcao: funcaoDaPessoa(p)?.Nome || "", caps };
  });
}
...
async function exportar(esc, pessoas, datas) {
  try {
    ...
    const gr = [["Pessoa", "Função", "Empresa", ...rotDias]];
    pessoas.forEach((p, i) => gr.push([p.nome, p.funcao || "", p.empresa || "", ...esc.grelha[i].map(v => v === "F" ? "Folga" : v === "EX" ? "Excesso" : (ROLES.find(r => r.key === v)?.rotulo || v))]));
...
function exportarFB(mes, need, porOutlet, esc, pessoas) {
  (async () => {
    try {
      ...
      const gr = [["Pessoa", "Função", "Empresa", ...DIAS]];
      pessoas.forEach((p, i) => gr.push([p.Nome, funcaoDaPessoa(p)?.Nome || "", p.EmpresaAgencia || "", ...esc.grelha[i].map(v => fbTexto(v))]));
      X.utils.book_append_sheet(wb, X.utils.aoa_to_sheet(gr), "Escala");
