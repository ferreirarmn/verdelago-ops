...
function secaoGrelha(esc, pessoas, datas, onEdit) {
  ...
  const opcoesDe = p => [
    ...
    ...ROLES.filter(r => !r.cap || p.caps[r.cap]).map(r => ({ val: r.key, txt: r.rotulo || r.key })),
    { val: "EX", txt: "Excesso" }
  ];
  const linhas = pessoas.map((p, i) => el("tr", {},
    ...
    ...esc.grelha[i].map((v, col) => {
      ...
      return celulaEditavel({ valor: v, opcoes: opcoesDe(p), marcada, ab, cls, aoEscolher: val => onEdit(data, p.nome, val) });
    })));
...
function fbGerar(pessoas, need, mes) {
  ...
  for (let d = 0; d < 7; d++) {
    ...
    const assign = {}, gap = {};
    for (const area of FB_AREAS) {
      const cnt = pessoas.reduce((s, _, i) => s + (grelha[i][d] === area.key ? 1 : 0), 0);
...
function renderFB(self, core, alvo) {
  ...
  // escala por pessoa (editável)
  const headP = el("tr", {}, el("th", { class: "rot" }, "Pessoa"), ...DIAS.map(d => el("th", {}, d)));
  const opcoesFB = [{ val: "__auto", txt: "Automático" }, { val: "F", txt: "Folga" },
    ...FB_AREAS.map(a => ({ val: a.key, txt: a.key })), { val: "EX", txt: "Excesso" }];
  const linhasP = pessoas.map((p, i) => el("tr", {},
    ...
    ...esc.grelha[i].map((v, d) => {
      ...
      return celulaEditavel({ valor: v, opcoes: opcoesFB, marcada, ab, cls,
        aoEscolher: val => { if (val !== null) definirAjuste(chaveFB(mes, d, p.Nome), val); self.render(core, alvo); } });
    })));
