...
function previsaoHSK(mesNome) {
  ...
  const split = cfg.valet_split != null ? Number(cfg.valet_split) : 0.5;   // manhã/tarde
  const valetTot = Math.max(Number(fx.min_valet) || 1, Math.ceil(rn * pond("Valet") / prod));
  const valetM = Math.round(valetTot * split), valetT = valetTot - valetM;
  ...
  const f = {
    ...
    "Valete (manhã)": [valetM, true],
    "Valete (tarde)": [valetT, true],
...
function vistaPrevisao(corpo, self, core, alvo) {
  ...
  // quadro fixo de pessoas por função (reparte o valete pelo split)
  const q = (M.params && M.params.hsk && M.params.hsk.quadro) || {};
  const vsplit = Number((M.params && M.params.hsk && M.params.hsk.valet_split) ?? 0.5);
  const qvM = Math.round((q["Valete"] || 0) * vsplit), qvT = (q["Valete"] || 0) - qvM;
  const quadro = {
    "Andares": q["Andares"] || 0, "Turndown": q["Turndown"] || 0,
    "Valete (manhã)": qvM, "Valete (tarde)": qvT,
    "Áreas comuns": q["Áreas comuns"] || 0, "Lavandaria": q["Lavandaria"] || 0,
    "Governanta": q["Governanta"] || 0
  };
  const totQuadro = Object.values(quadro).reduce((a, b) => a + b, 0);

  // tabela HSK por função: QUADRO FIXO (pessoas) vs NECESSIDADE calculada
  const secHSK = hsk ? el("div", { class: "orc-sec" },
    el("h3", {}, "Housekeeping — quadro fixo vs necessidade por função"),
    el("table", { class: "orc-tab" },
      el("thead", {}, el("tr", {}, el("th", {}, "Função"), el("th", { class: "num" }, "Quadro (pessoas)"), el("th", { class: "num" }, "Necessidade (pos./dia)"), el("th", { class: "num" }, "Δ"), el("th", {}, "Tipo"))),
      el("tbody", {}, ...Object.entries(hsk.funcoes).map(([f, [pos, varia]]) => {
        const qf = quadro[f] || 0, d = qf - pos;
        return el("tr", {}, el("td", {}, f), el("td", { class: "num" }, "" + qf), el("td", { class: "num" }, "" + pos),
          el("td", { class: "num", style: "color:" + (d < 0 ? "var(--danger)" : "var(--teal)") }, (d >= 0 ? "+" : "") + d),
          el("td", {}, varia ? "variável" : "fixo"));
      })),
      el("tfoot", {}, el("tr", {}, el("td", {}, "Total"), el("td", { class: "num" }, "" + totQuadro), el("td", { class: "num" }, hsk.posicoes + " pos · " + hsk.hc + " HC"), el("td", {}, ""), el("td", {}, ""))))) : null;
