...
async function aplicar(core, alvo, btn, log, presOk) {
  ...
  if (cr) {
    ...
    function payload(c, idTitle, excluir) {
      ...
      // ocupante
      if (!excluir.has("PessoaID") && c.Nome && zonaPid) {
    ...
    let excluir = new Set();
    let calibrado = false;
    let criadas = 0, semZona = 0, n = 1;

    for (const c of cr.camas) {
      if (!zonaPorNome(c.Edificio)) { semZona++; continue; }
      const idTitle = "C" + String(n).padStart(3, "0");

      if (!calibrado) {
        // tenta com tudo; se falhar, remove campos por ordem até criar
        let ok = false, tentativa = new Set();
        for (let i = 0; i <= ordemRemover.length; i++) {
          try {
            await graph.criarItem("Camas", payload(c, idTitle, tentativa));
            excluir = new Set(tentativa); calibrado = true; ok = true; criadas++; n++;
            if (excluir.size) linha("Aviso: ignorei o(s) campo(s) " + [...excluir].join(", ") + " (o SharePoint recusou-os).", false);
            else linha("Calibração ok — todos os campos aceites.");
            break;
          } catch (e) {
            if (i < ordemRemover.length) tentativa.add(ordemRemover[i]);
            else { linha("Não consegui criar nem a 1ª cama: " + e.message, false); nErr++; }
          }
        }
        if (!ok) break;
      } else {
        try { await graph.criarItem("Camas", payload(c, idTitle, excluir)); criadas++; n++; }
        catch (e) { linha("Falha ao criar " + idTitle + ": " + e.message, false); nErr++; if (nErr > 10) { linha("Demasiados erros — parei.", false); break; } }
      }
    }
    linha("Camas criadas: " + criadas + (semZona ? (" (" + semZona + " sem zona)") : ""));
    nOk += criadas;
  }
