...
async function aplicar(core, alvo, btn, log, presOk) {
  ...
  if (cr) {
    ...
    // 4c) criar as novas (usando os nomes internos reais)
    let criadas = 0, semZona = 0;
    let n = 1;
    for (const c of cr.camas) {
      const zona = zonaPorNome(c.Edificio);
      if (!zona) { semZona++; continue; }
      const campos = { Title: "C" + String(n).padStart(3, "0") };
      const set = (logico, valor) => { const cc = col(logico); if (cc && valor !== "" && valor != null) campos[cc.name] = valor; };
      set("Quarto", c.Quarto);
      set("Numero", c.Numero);
      set("Estado", c.Estado || "Livre");
      if (c.DataChegada) set("DataChegada", c.DataChegada);
      // zona (lookup ou texto)
      if (colZona) {
        if (zonaLookup) campos[colZona.name + "LookupId"] = Number(zona._id);
        else campos[colZona.name] = zona.Title;
      }
      // ocupante (texto)
      if (c.Nome) { const pe = pessoaPorNome(c.Nome); if (pe) { const pc = col("PessoaID"); if (pc) campos[pc.name] = pe.Title; } }
      try { await graph.criarItem("Camas", campos); criadas++; n++; }
      catch (e) { linha("Falha ao criar cama " + campos.Title + ": " + e.message, false); nErr++; if (nErr > 8) break; }
    }
