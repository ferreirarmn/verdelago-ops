...
async function aplicar(core, alvo, btn, log, presOk) {
  ...
  if (cr) {
    ...
    // calibra na 1ª escrita: tenta com tudo e vai retirando campos até aceitar
    async function comCalibracao(c, idTitle, existingId) {
      if (calibrado) return escrever(payload(c, idTitle, excluir), existingId);
      let tentativa = new Set();
      for (let k = 0; k <= ordemRemover.length; k++) {
        try {
          const r = await escrever(payload(c, idTitle, tentativa), existingId);
          excluir = new Set(tentativa); calibrado = true;
          linha(excluir.size ? ("Aviso: ignorei o(s) campo(s) " + [...excluir].join(", ") + " (o SharePoint recusou-os).") : "Calibração ok — todos os campos aceites.", !excluir.size);
          return r;
        } catch (e) {
    ...
    let criadas = 0, atualizadas = 0, apagadas = 0;
    linha("A gravar " + novas.length + " camas" + (semZona ? (" (" + semZona + " sem zona, ignoradas)") : "") + "…");

    for (let idx = 0; idx < total; idx++) {
      const idTitle = "C" + String(idx + 1).padStart(3, "0");
      try {
        if (idx < novas.length) {
          const existingId = idx < existentes.length ? existentes[idx]._id : null;
          await comCalibracao(novas[idx], idTitle, existingId);
          if (existingId) atualizadas++; else criadas++;
        } else {
          await graph.apagarItem("Camas", existentes[idx]._id); apagadas++;
        }
      } catch (e) {
        linha("Falha na cama " + idTitle + ": " + e.message, false); nErr++;
        if (nErr > 15) { linha("Demasiados erros seguidos — parei para não bloquear.", false); break; }
      }
      if ((idx + 1) % 10 === 0 || idx + 1 === total) {
        linha("Progresso: " + (idx + 1) + "/" + total + " · " + criadas + " criadas, " + atualizadas + " atualizadas, " + apagadas + " apagadas");
      }
    }
