...
function vistaHSK(self, core, alvo) {
  ...
  const fixos = el("div", { class: "par-fixos" },
    ...
    campoNum(h, "valet_split", "Valete: split manhã (0–1)", "0.05"));

  // quadro fixo de pessoas por função (mostrado lado a lado na previsão)
  const quad = h.quadro || (h.quadro = { "Andares": 0, "Valete": 0, "Governanta": 0, "Turndown": 0, "Lavandaria": 0, "Áreas comuns": 0 });
  const quadro = el("div", { class: "par-fixos" },
    ...["Andares", "Valete", "Governanta", "Turndown", "Lavandaria", "Áreas comuns"].map(k => campoNum(quad, k, k)));

  const btn = el("button", { class: "btn", onclick: () => guardar("hsk", { tipos, mix_tipologias: mix, horas_produtivas: h.horas_produtivas, fixos: fx, valet_split: h.valet_split, quadro: quad }, btn, self, core, alvo) }, "Guardar parâmetros HSK");
  ...
  return el("div", {},
    ...
    el("div", { class: "par-sec" }, el("h3", {}, "Quadro fixo de pessoas por função"),
      el("p", { class: "aj" }, "Pessoas fixas por função (do simulador). Aparecem na Previsão ao lado da necessidade calculada. O valete reparte-se manhã/tarde pelo split acima."), quadro),
