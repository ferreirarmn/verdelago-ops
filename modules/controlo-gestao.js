...
import { carregarCorrespondencias, canonDepartamento, canonFuncao, itensCorrespondencia } from "../core/correspondencias.js";
...
export const moduloControloGestao = {
  ...
  render(core, alvo) {
    ...
    const atualizarBtn = el("button", { class: "btn-sec", onclick: async () => { M.plano = null; M.erro = null; _ent = undefined; _sai = undefined; await self.init(); self.render(core, alvo); } }, "↻ Atualizar");
...
function vista(corpo) {
  ...
  const nCorrF = itensCorrespondencia().filter(it => (it.TipoCorr || "Função") === "Função").length;
  const semDatas = !campoData("ent");
  const nota = el("div", { class: "mod-nota" },
    (dim === "funcao" ? nCorrF + " correspondência(s) de função ativas. " : "") +
    (semDatas
