...
function vista(corpo) {
  ...
  const realTot = campo === "custo" ? real.custo : real.hc;
  ...
  const linhas = grupos.map(k => {
    const a = vP(ini, k), b = vP(atu, k), r = vR(k), dv = r - b;
    return el("tr", {},
      el("td", { class: "rot" }, k), el("td", {}, fmt(a)), el("td", {}, fmt(b)), el("td", {}, fmt(r)),
      el("td", { style: "color:" + (dv > 0 ? "var(--danger)" : "var(--teal)") }, (dv >= 0 ? "+" : "") + fmt(dv)));
  });
  const totRow = el("tr", { style: "font-weight:700;border-top:2px solid var(--line)" },
    el("td", { class: "rot" }, "Total"), el("td", {}, fmt(iniTot)), el("td", {}, fmt(atuTot)), el("td", {}, fmt(realTot)),
    el("td", {}, (realTot - atuTot >= 0 ? "+" : "") + fmt(realTot - atuTot)));
