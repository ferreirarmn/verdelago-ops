...
export function criarModuloDepartamento(cfg) {
  ...
  function vistaEquipa(corpo, core, alvo) {
    ...
    M.filtro = M.filtro || { dep: "", vinc: "", estado: "", fun: "" };
    if (!("fun" in M.filtro)) M.filtro.fun = "";
    ...
    const funcs = [...new Set(pessoas.map(p => { const f = funcaoDaPessoa(p); return f ? f.Nome : null; }).filter(Boolean))].sort((a, b) => a.localeCompare(b, "pt"));
    ...
    const selFun = el("select", { class: "campo", style: "max-width:200px", onchange: e => { M.filtro.fun = e.target.value; render(core, alvo); } }, ...opcoes(funcs, M.filtro.fun));
    ...
    if (geral) acoes.push(
      ...
      el("label", { style: "font-size:12px;color:var(--mut)" }, "Função "), selFun,
    ...
    acoes.push(el("button", { class: "btn-sec", onclick: () => exportarPessoasExcel(cfg, geral, COLUNAS_EXTRA || [], M) }, "Exportar Excel"));
    ...
    const linhas = pessoas
      ...
      .map(p => {
        ...
        tr.dataset.fun = f ? f.Nome : "";
        return tr;
      });

    const filtrar = () => {
      const q = norm(busca.value); M.busca = busca.value;
      const { dep, vinc, estado, fun } = M.filtro;
      let visiveis = 0;
      for (const tr of linhas) {
        const ok = (!q || tr.dataset.nome.includes(q)) &&
          (!dep || tr.dataset.dep === dep) && (!vinc || tr.dataset.vinc === vinc) &&
          (!estado || tr.dataset.estado === estado) && (!fun || tr.dataset.fun === fun);
        tr.style.display = ok ? "" : "none"; if (ok) visiveis++;
      }
      const filtrado = q || dep || vinc || estado || fun;
...
async function exportarPessoasExcel(cfg, geral, extra, M) {
  try {
    ...
    // pessoas: respeita "mostrar inativas", o filtro de função e (no geral) dept/vínculo/estado
    let ps = cfg.selPessoas(M.verInativas);
    if (M.filtro) {
      const { dep, vinc, estado, fun } = M.filtro;
      ps = ps.filter(p => {
        const f = funcaoDaPessoa(p);
        if (fun && (f ? f.Nome : "") !== fun) return false;
        if (geral && ((dep && (moduloDaPessoa(p) || "—") !== dep) || (vinc && (p.Vinculo || "") !== vinc) || (estado && (p.Estado || "Ativo") !== estado))) return false;
        return true;
      });
    }
