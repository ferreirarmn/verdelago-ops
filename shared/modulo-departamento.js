...
const ehCampoData = c => /data|date|nasc|admiss|in[ií]cio|fim|termo/i.test(c.name + " " + (c.displayName || ""));
...
export function criarModuloDepartamento(cfg) {
  ...
  function editorPessoa(p, core, alvo) {
    ...
    // Campos extra descobertos na Lista Pessoas (datas, contactos, etc.)
    const extras = COLUNAS_EXTRA || [];
    const inputsExtra = {};
    const camposExtraEls = extras.map(c => {
      const data = ehCampoData(c);
      const valAtual = p ? p[c.name] : "";
      const input = el("input", { class: "campo", type: data ? "date" : "text",
        value: data ? isoParaData(valAtual) : (valAtual ?? "") });
      inputsExtra[c.name] = { input, data };
    ...
    const guardar = el("button", { class: "btn", onclick: async () => {
      ...
      // juntar campos extra (datas em ISO; texto pode ser limpo)
      for (const [name, { input, data }] of Object.entries(inputsExtra)) {
        const v = input.value.trim();
        if (data) { if (v) campos[name] = new Date(v).toISOString(); }
        else campos[name] = v;
      }
